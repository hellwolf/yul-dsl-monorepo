...
class LinearPipelinePlus a expr where
  (&+) :: forall. a ⊸ expr

class LinearPipelineMinus a expr where
  (&-) :: forall. a ⊸ expr

infixl 1 &-, &+

newtype Use'L a b r v = MkUse'L (P'V v r a, P'V v r b)
use'l :: forall a b r v. (YulO3 a b r)
      => P'V v r a ⊸ (P'V v r a ⊸ P'V v r b) ⊸ Use'L a b r v
use'l a f = split (copy a) & \(a', a'') -> MkUse'L (a', f a'')
instance forall a b c r v.
         ( YulO4 r a b c
         ) => LinearPipelinePlus (Use'L a b r v)
         ((P'V v r a ⊸ P'V v r b ⊸ P'V v r c) ⊸ P'V v r c) where
  (MkUse'L (a, b)) &+ f = f a b
instance forall a b c r v.
         ( YulO4 r a b c
         ) => LinearPipelineMinus (Use'L a b r v)
         ((P'V v r a ⊸ P'V v r c) ⊸ P'V v r c) where
...
instance forall r b v.
         ( YulO2 r b
         ) => LinearPipelinePlus (Dis'L r v)
         ((P'V v r () ⊸ P'V v r b) ⊸ P'V v r b) where
  (MkDis'L u) &+ f = f u
instance forall r b v.
         ( YulO2 r b
         ) => LinearPipelineMinus (Dis'L r v)
         (P'V v r b ⊸ P'V v r b) where
